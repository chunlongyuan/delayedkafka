...
type Message struct {
	...
	Topic       string    `json:"topic" gorm:"type:varchar(100);column:topic;not null;default 0;comment:'消息主题'"`     // kafka 接收用的 topic
	...
	Status      int8      `json:"-" gorm:"column:status;not null;default 0;comment:'消息状态: 0:待消费 1:已删除 2:已消费'"`       // 消费时刻的毫秒时间戳
	...
	UpdatedAt   time.Time `json:"-" gorm:"column:updated_at;type:datetime;not null;default:current_timestamp;comment:'更新时间'"`
}
